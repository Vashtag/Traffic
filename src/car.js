...
class Car {
  ...
  update(dt, allCars) {
    ...
    if (targetNode.control?.type === 'light') {
      if (targetNode.control.state === 'red' && distRemaining < STOP_DISTANCE) mustStop = true;
    }
    ...
    if (targetNode.control?.type === 'roundabout' && distRemaining < STOP_DISTANCE * 1.5) {
      for (const other of allCars) {
        ...
        if (other.path[other.pathIdx]?.id !== targetNode.id) continue;
        if (other.edge.id === this.edge.id) continue;
        const otherRemaining = other.edge.length * (1 - other.t);
        if (otherRemaining < distRemaining - 5) { mustStop = true; break; }
      }
    }
    ...
    for (const other of allCars) {
      ...
      if (sameDir && other.t > this.t && (other.t - this.t) * edgeLen < MIN_GAP) {
        mustStop = true; break;
      }
    }

    const targetSpeed = (mustStop) ? 0 : this.maxSpeed;
